...
train_pipeline = [
    ...
    dict(type='CSPMaps', radius=8, with_width=width, stride=4, regress_range=(-1, 1e8), image_shape=img_scale, num_classes=10),
...
data = dict(
    ...
    train=dict(
        type="CocoDataset",
        classes=['car', 'bus', 'rider', 'bicycle', 'person', 'motorcycle', 'truck', 'train',],
        ann_file="datasets/CityScapes/train.json",
    ...
    val=dict(
        type="CocoDataset",
        classes=['car', 'bus', 'rider', 'bicycle', 'person', 'motorcycle', 'truck', 'train',],
        ann_file="datasets/CityScapes/val.json",
        img_prefix="/netscratch/hkhan/cp/leftImg8bit_trainvaltest/",
...
lr_config = dict(step=[80, 110], policy='step', gamma=1/3, warmup='constant', warmup_iters=250, warmup_ratio=1.0/3,)
