...
model = dict(
    ...
    bbox_head=dict(
        ...
        loss_ttc=dict(type='QTTCLoss', loss_weight=0.25),
    ),
    train_cfg=dict(
        rcnn=dict(
            assigner=dict(
...
train_pipeline = [
    ...
    dict(type='CSPMaps', radius=2, with_width=width, stride=4, regress_range=(-1, 1e8), image_shape=img_scale, with_ttc=True, num_classes=num_classes, ttc_mode='quantized', ttc_bins=1),
...
test_pipeline = [
    ...
    dict(
        ...
        transforms=[
            ...
            dict(type='RemoveSmallBoxes', min_box_size=8, min_gt_box_size=8),
            dict(type='Normalize', **img_norm_cfg),
            dict(type='CSPMaps', radius=2, with_width=width, stride=4, regress_range=(-1, 1e8), with_ttc=True, num_classes=num_classes, ttc_mode='quantized', ttc_bins=1),
...
data_root = '/netscratch/hkhan/kitti/data/training/image_02/'
data = dict(
    samples_per_gpu=12,
    workers_per_gpu=2,
    train=dict(
