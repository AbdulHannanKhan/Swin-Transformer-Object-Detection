...
model = dict(
    ...
    bbox_head=dict(
        ...
        loss_ttc=dict(type='L1RoDLoss', loss_weight=0.25),
    ),
    train_cfg=dict(
        rcnn=dict(
            assigner=dict(
...
train_pipeline = [
    ...
    dict(type='CSPMaps', radius=2, with_width=width, stride=4, regress_range=(-1, 1e8), image_shape=img_scale, with_ttc=True, num_classes=num_classes),
...
test_pipeline = [
    ...
    dict(
        ...
        transforms=[
            ...
            dict(type='RemoveSmallBoxes', min_box_size=8, min_gt_box_size=8),
            dict(type='Normalize', **img_norm_cfg),
            dict(type='CSPMaps', radius=2, with_width=width, stride=4, regress_range=(-1, 1e8), with_ttc=True, num_classes=num_classes),
    ...
    hooks=[
        ...
        dict(
            ...
            init_kwargs=dict(
                entity="hannankhan",
                project="KittiTTC",
                name="csp_rod_car_ped_4x16_radius_2_70_nus_68",
                config=dict(
                    work_dirs="${work_dir}",
