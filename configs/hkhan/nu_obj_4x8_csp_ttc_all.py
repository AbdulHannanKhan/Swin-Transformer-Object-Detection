...
train_pipeline = [
    ...
    dict(type='CSPMaps', radius=2, with_width=width, stride=4, regress_range=(-1, 1e8), image_shape=img_scale, with_ttc=True, num_classes=10),
...
test_pipeline = [
    ...
    dict(
        ...
        transforms=[
            ...
            dict(type='RemoveSmallBoxes', min_box_size=8, min_gt_box_size=8),
            dict(type='Normalize', **img_norm_cfg),
            dict(type='CSPMaps', radius=2, with_width=width, stride=4, regress_range=(-1, 1e8), with_ttc=True, num_classes=10),
            dict(type='ImageToTensor', keys=['img', 'ttc_maps']),
            dict(type='Collect', keys=['img', 'gt_bboxes', 'gt_labels', 'classification_maps', 'scale_maps', 'offset_maps', 'ttc_maps', 'gt_labels']),
       ],
    ),
]
...
data = dict(
    ...
    train=dict(
        type="CocoTTCDataset",
        classes=classes,
        ann_file="/netscratch/hkhan/nu/fullc/nuscenes_infos_train_mono3d.coco.json",
    ...
    val=dict(
        type="CocoTTCDataset",
        classes=classes,
        ann_file="/netscratch/hkhan/nu/fullc/nuscenes_infos_val_mono3d.coco.json",
        img_prefix="/netscratch/hkhan/nuscenes/raw/",
...
lr_config = dict(step=[60, 74], policy='step', warmup='constant', warmup_iters=250, warmup_ratio=1.0/3,)
...
# resume_from="./work_dirs/nu_obj_4x8_csp_ttc_front/epoch_16.pth"
