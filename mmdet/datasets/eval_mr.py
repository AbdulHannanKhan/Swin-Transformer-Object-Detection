...
class COCOeval:
    ...
    def _prepare(self, id_setup):
        ...
        p = self.params
        if p.useCats:
            gts=copy.deepcopy(self.cocoGt.loadAnns(self.cocoGt.getAnnIds(imgIds=p.imgIds, catIds=p.catIds)))
            dts=self.cocoDt.loadAnns(self.cocoDt.getAnnIds(imgIds=p.imgIds, catIds=p.catIds))
        else:
            gts=copy.deepcopy(self.cocoGt.loadAnns(self.cocoGt.getAnnIds(imgIds=p.imgIds)))
        ...
        for gt in gts:
            ...
            gt['ignore'] = 1 if (gt['bbox'][3] < self.params.HtRng[id_setup][0] or gt['bbox'][3] > self.params.HtRng[id_setup][1]) or \
    ...
    def computeIoU(self, imgId, catId):
        ...
        elif p.iouType == 'vis_bbox':
            g = [g['vis_bbox'] for g in gt]
            d = [d['bbox'] for d in dt]
        else:
    ...
    def accumulate(self, p = None):
        ...
        vis_dt_ids = np.empty((T, R, K, M), dtype=list)
        ...
        for k, k0 in enumerate(k_list):
            ...
            for m, maxDet in enumerate(m_list):
                ...
                dtIds = np.concatenate([e['dtIds'][:maxDet] for e in E], axis=0)[inds]
                ...
                for t, (tp, fp) in enumerate(zip(tps, fps)):
                    ...
                    try:
                        for ri, pi in enumerate(inds):
                            q[ri] = recall[pi]
                            vis_dt_ids[t, ri, k, m] = _dtIds[:pi].tolist()
                    except:
                        pass
                    ys[t,:,k,m] = np.array(q)
        self.eval = {
    ...
    def summarize(self,id_setup, res_file=None):
        ...
        def _summarize(iouThr=None, maxDets=100 ):
            ...
            #print("LAMR: ", mean_s)
            self.stats.append(mean_s*100)
            logstr = iStr.format(titleStr, typeStr,setupStr, iouStr, heightStr, occlStr, mean_s*100)
            #print(logstr)
            # res_file.write(iStr.format(titleStr, typeStr,setupStr, iouStr, heightStr, occlStr, mean_s*100))
            if res_file is not None:
                res_file.write(logstr)
                res_file.write('\n')
            return logstr
...
class Params:
    ...
    def setDetParams(self):
        ...
        self.recThrs = np.linspace(.0, 1.00, int(round((1.00 - .0) / .01) + 1), endpoint=True)
    ...
    def __init__(self, iouType='segm'):
        if iouType == 'segm' or iouType == 'bbox' or iouType == 'vis_bbox':
            self.setDetParams()
        else:
